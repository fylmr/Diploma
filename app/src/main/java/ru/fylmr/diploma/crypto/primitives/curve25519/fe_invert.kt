 ...
 * fe_invert
 */
fun feInversion(z: FieldElement): FieldElement {
    ...
    feSquare(t1.bytes, t0.bytes)
    var i = 1
    while (i < 2) {
        feSquare(t1.bytes, t1.bytes)
        ++i
    }
    ...
    feSquare(t2.bytes, t1.bytes)
    i = 1
    while (i < 5) {
        feSquare(t2.bytes, t2.bytes)
        ++i
    }
    ...
    feSquare(t3.bytes, t2.bytes)
    i = 1
    while (i < 20) {
        feSquare(t3.bytes, t3.bytes)
        ++i
    }
