...
static int ge_frombytes_negate_vartime(GroupElemExtended h, byte[] s)
{
  int[] u = new int[10];
  int[] v = new int[10];
  int[] v3 = new int[10];
  ...
  fe_sq.fe_sq(u,h.Y);
  fe_mul(v,u,d);
  FieldElementSubtraction.fieldElementSubtraction(u,u,h.Z);       /* u = y^2-1 */
  FieldElementAddition.fieldElementAddition(v,v,h.Z);       /* v = dy^2+1 */
  ...
  fe_sq.fe_sq(vxx,h.X);
  fe_mul(vxx,vxx,v);
  FieldElementSubtraction.fieldElementSubtraction(check,vxx,u);    /* vx^2-u */
  if (fe_isnonzero.fe_isnonzero(check) != 0) {
    FieldElementAddition.fieldElementAddition(check,vxx,u);  /* vx^2+u */
